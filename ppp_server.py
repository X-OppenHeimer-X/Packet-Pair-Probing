...
if c.FIXED_BANDWIDTH == True and c.FIXED_PACKET_SIZE == False:
    ...
    while True and trial_number < trials:  ##loop ends after all trials are completed.
        ...
        packet1_receiving_time = packet1_receiving_time + (
            c.sizes[trial_number] / c.bandwidth
        )
        ...
        packet2_receiving_time = packet2_receiving_time + (
            c.sizes[trial_number] / c.bandwidth
        )
        print(f"Packet 2 for trial number #{trial_number+1} received")
        final_dispersions_list_fixed_bandwidth.append(
            packet2_receiving_time - packet1_receiving_time
        )
        trial_number += 1
    ...
    while True and trial_number < 1:  ### Loop ends after one packet is received.
        ...
        packet1_receiving_time = time.time()
        print(
            f"Packet 1 for trial number #{trial_number+1} for bandwidth {bandwidth}Mbits/sec received"
        )
        packet_2, client_address = server_socket.recvfrom(1024)
        packet2_receiving_time = time.time()
        print(
            f"Packet 2 for trial number #{trial_number+1} for bandwidth {bandwidth}Mbits/sec received"
        )
        final_dispersions_list_fixed_packet_size.append(
            max(
                c.packet_size / (bandwidth * 1000000),
                (packet2_receiving_time - packet1_receiving_time),
            )
        )
        trial_number += 1
    ...
    print(
        "The configuration is incorrect.Please fix the configuration in config.py file"
    )
    sys.exit()
