...
def configs_okay(a, b):
    if bool(a) != bool(b):
        ...
        print(
            "The configurations are not set properly.Please check the config.py file."
        )
        sys.exit()
...
def myNetwork(bandwidth):
    ...
    info("* Add hosts\n")
    h1 = net.addHost(
        "h1", cls=Host, ip="192.168.0.103/24", defaultRoute="via 192.168.0.1"
    )
    h2 = net.addHost(
        "h2", cls=Host, ip="192.168.1.2/24", defaultRoute="via 192.168.1.1"
    )
    h3 = net.addHost(
        "h3", cls=Host, ip="192.168.3.11/24", defaultRoute="via 192.168.3.1"
    )
    h4 = net.addHost(
        "h4", cls=Host, ip="192.168.4.2/24", defaultRoute="via 192.168.4.1"
    )
